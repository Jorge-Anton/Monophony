...
part 'get_lists.g.dart';

@Riverpod(keepAlive: true)
Future<List<ListModel>?> getLists(GetListsRef ref, String query) async {
  ...
  final body = '''{"context":{"client":{"clientName":"WEB_REMIX","clientVersion":"1.20220918","platform":"DESKTOP","hl":"en","visitorData":"CgtEUlRINDFjdm1YayjX1pSaBg%3D%3D"}},"query":"$query","params":"EgeKAQQoAEABagoQAxAEEAoQCRAF"}''';
  ...
  return json.contents?.tabbedSearchResultsRenderer?.tabs?.firstOrNull?.tabRenderer?.content?.sectionListRenderer?.contents?.elementAtOrNull(1)?.musicShelfRenderer?.contents?.nonNulls.map((e) => e.musicResponsiveListItemRenderer).nonNulls.map((e) => ListModel.fromMusicResponsiveListItemRenderer(e)).toList();
}