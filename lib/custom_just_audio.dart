...
@experimental
class LockCachingResolvingAudioSource extends StreamAudioSource {
  final String uniqueId;
  final ResolveSoundUrl resolveSoundUrl;
  ...
  /// neeeds a function that returns that [uri] to be provided.
  /// 

  /// Creates a [LockCachingResolvingAudioSource] to that provides [uri] to the player
  ...
  /// If headers are set, just_audio will create a cleartext local HTTP proxy on
  /// your device to forward HTTP requests with headers included.
  LockCachingResolvingAudioSource({
    required this.uniqueId,
    required this.resolveSoundUrl,
    this.headers,
    File? cacheFile,
    dynamic tag,
            ...
            cacheFile != null ? Future.value(cacheFile) : _getCacheFile(uniqueId),
        super(tag: tag) {
    _init();
  }
  ...
  static Future<File> _getCacheFile(final String uniqueId) async => File(p.joinAll([
        (await _getCacheDir()).path, uniqueId,
      ]));
  ...
  Future<HttpClientResponse> _fetch() async {
    ...
    subscription = response.listen((data) async {
      ...
      if ((await _partialCacheFile).existsSync()) {
